...
def comment_page(post_id):
    ...
    if request.method == 'POST':
            ...
            with get_db_connection() as conn:
                with conn.cursor() as cursor:
                    try:
                        ...
                        name = name_row[0] if name_row and len(name_row) > 0 else 'Unknown'

                        # Update the comment count for the post
                        cursor.execute("SELECT comment_count FROM posts WHERE id = %s", (post_id,))
                        value_row = cursor.fetchone()
                        value = value_row[0] if value_row and len(value_row) > 0 else 0
                        value += 1
        ...
        return redirect(url_for('comment_page_view', post_id=post_id, username=session.get('username')))
