...
def signup_user():
    ...
    try:
        with get_db_connection() as conn:
            ...
            # Assumed users table column order: id, name, username, password, email, profile_picture
            # Check if the username, email, or name already exists in the database
            existing_user = user_exists(cursor, username, email, name)
            if existing_user:
                if len(existing_user) > 2 and existing_user[2] == username:
                    flash('Username already exists! 😞', 'danger')
                elif len(existing_user) > 4 and existing_user[4] == email:
